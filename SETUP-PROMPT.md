...
| **사람만 가능(AI 요청)** | OpenAI 키 발급·입력, GUI 권한 승인, graph-lab "그래프 구축" 버튼 클릭 |
...
| **Windows 지원** | `run-neo4j.ps1` 추가로 Windows graph-lab 지원(2026-05-20 실측). 실패 시 Docker Desktop 실행 확인 후 "그래프 구축" 재클릭 |
| **6→7단계 분리 이유** | LLM은 터미널은 제어해도 브라우저 버튼 클릭은 불가 → 인프라는 5단계 사전 기동, 클릭만 사람에게 |
