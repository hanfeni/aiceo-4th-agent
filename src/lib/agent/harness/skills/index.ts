...
/**
 * 하네스 요소: SKILL (deepagents SkillsMiddleware) — PoC.
 *
 ...
 * 노출된다. PoC 는 rootDir 를 skills/ 디렉토리로 격리해 노출면을 스킬로만
 * 한정한다. skill 소스 경로는 그 root 기준 상대(/deep-web-research/).
 */
...
/**
 * skill 소스 경로 목록 (backend rootDir 기준 POSIX 절대경로).
 * 추가 시 이 배열에 1줄 (skills/<name>/SKILL.md → "/<name>/").
 */
export const SKILL_SOURCES = ["/deep-web-research/"] as const;
