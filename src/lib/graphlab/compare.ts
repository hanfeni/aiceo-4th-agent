...
async function* runSqlPanel(
  ...
  const sys =
    ...
    "(accession, cusip, issuer, value_usd_k, shares, put_call) " +
    "하나뿐입니다. put_call: ''=현물 / 'Call' / 'Put'.\n" +
    "규칙:\n" +
...
async function* runGraphRagPanel(
  ...
  const sys =
    ...
    "(:Company {cusip, name, holder_count, total_value_k})\n" +
    "  └ holder_count = 이 종목을 보유한 13F 기관 수(인기/crowding " +
    "지표 — '허브 종목·인기 종목'은 count() 대신 이 속성 직조회).\n" +
    "  └ total_value_k = 전체 보유가치 합계(USD 천 단위).\n" +
    "(:Position {accession, cusip, value_usd_k, shares, put_call})\n" +
    "  └ put_call: ''=현물 / 'Call' / 'Put' (옵션 포지션 질의용).\n" +
