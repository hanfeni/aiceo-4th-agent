...
| Docker Desktop | AI 자동 점검·설치 | 미설치 시 설치·기동. **단, 첫 설치 시 동의·권한 절차는 사람이 처리** (아래 ⚠️) |
...
>
> 🐳 **Docker Desktop 을 이번에 처음 설치한다면** — AI가 설치까지는 자동으로
> 하지만, 설치 직후 **첫 실행에서 사람이 직접 통과해야 하는 GUI 관문**이
> 있습니다. AI는 설치를 진행하다 이 지점에서 멈추고 무엇을 누르라고 안내합니다.
> 화면이 뜨면 아래를 순서대로 처리해 주세요(약 1~2분):
>
> 1. **서비스 약관(Service Agreement) 동의** — `Accept` 클릭
> 2. **추천/기본 설정 화면** — 그대로 `Finish` 또는 `Continue`
> 3. **macOS 권한 요청 다이얼로그** — 비밀번호 입력 후 `OK` (Docker 가 시스템 권한을 요구)
> 4. **로그인(Sign in) 유도 화면** — 계정 없이 `Skip` / `Continue without signing in` 가능
>
> 이 관문을 통과해 메뉴바 고래 아이콘이 `running` 이 되면, AI에게 "동의 끝났어"
> 라고 알려주면 됩니다. AI가 자동으로 다음 단계를 이어서 진행합니다.
...
2. **단순 설명을 넘는 질문(왜 이렇게 동작하나, 옵션값이 정확히 뭔가, 에러 원인은 무엇인가)은 lecture-guide 만 믿지 말고 실제 코드를 함께 분석해 답한다.** lecture-guide 는 요약본이라 세부가 코드와 다를 수 있다. 항상 코드가 정답이다.
   ...
   - 도메인·데이터 소스 = src/lib(또는 src/server)의 searchlab/domains.ts, sqllab/domains.ts, graphlab/config.ts
...
### 챗 에이전트 (/chat)
deepagents 그래프 기반 SSE 스트리밍 멀티턴 챗. 우측 상단에 드롭다운 3개:
- **모델**: gpt-5.5 / gpt-5.4 / gpt-5.4-mini (기본 gpt-5.4-mini, 전부 OpenAI)
- **인덱스검색(도구)**: "안함" + 5개 도메인. 색인 안 된 도메인은 비활성(먼저 /index-lab 에서 색인 필요)
- **데이터조회 SQL(도구)**: "안함" + 5개 도메인. 미적재 도메인 선택 시 자동으로 SQLite 적재 후 활성화
- 도구 미선택=순수 챗(고정형) / 선택=자율형 에이전트(LLM이 검색·SQL 도구를 스스로 호출)
- ⚠️ 인덱스검색·SQL 도메인을 바꾸면 세션이 리프레시되어 진행 중 대화가 초기화된다.
- 실습 질문 예시: "강남구 주변 카페 많은 동네 알려줘"(상권), "뱅가드 보유 상위 종목"(graph-lab 별도)

### 하네스 구성 (/harness)
현재 챗에 적용된 하네스 요소(Skill / Subagent / 도구 / 시스템 인스트럭션 / 토글 상태)를 **읽기 전용으로 표시**하는 화면. 토글 자체는 환경변수(HARNESS_PLANNING/FILESYSTEM/SUBAGENTS/SKILLS)로 제어되며, 이 화면은 현황 확인용이다(설정 변경 UI 아님).

### DART 기업분석 (/dart)
금융감독원 DART 공시 기반 8관점 펀더멘털 분석(고정 파이프라인). 기업명 입력 + 관점 선택(종합/재무건전성/성장성/수익성/밸류에이션/지배구조/리스크/인력·조직) → SSE 리포트.
- ⚠️ 메뉴는 항상 활성(회색 아님). DART_API_KEY 미설정이면 **분석 실행 시점에 에러**가 난다(진입은 됨). opendart.fss.or.kr 무료 발급.

### 도메인 색인 (/index-lab)
도메인 문서를 OpenSearch에 색인. 검색 실습·챗 인덱스검색의 전제.
옵션: 도메인 5택1 / Nori 분해(mixed·discrete·none) / 임베딩 모델(3-small 1536d·3-large 3072d) / 문서 수(100·300·500·1000, 기본 300) / **청크 크기(0=안함(기본)·200·500·1000·2000·5000 토큰)** / overlap(100·200·500·1000). 색인 인덱스 목록·삭제(searchlab-* 만) 제공.

### 데이터 적재 (/data-load)
도메인 CSV를 받아 SQLite 테이블(sqllab_*)로 적재. Text-to-SQL 실습의 전제. 옵션은 적재 행수 상한(1000·5000·10000·20000) 하나.

### 메타 라벨링 실습 (/meta-lab)
LLM이 실제로 문서 분류 메타를 생성하고 그 시스템 인스트럭션을 화면에 노출. 작업 4종: 메타 라벨링 / 스키마 발굴 / 올인원(발굴→수렴→분류기픽스→실분류 자동 4단계) / 올인원 색인(+⑤메타 OpenSearch 색인). 문서 수 1·3·5·10(기본 3).

### 검색 실습 (/search-lab)
색인된 인덱스를 검색·RAG·Text-to-SQL 로 비교.
- **렉시컬(BM25·Nori)**: 키워드 정확 매칭 — 법조문·고유명사에 유리. BM25 가중치 프리셋 3종(균형·타이틀중심·본문중심)
- **벡터(임베딩)**: 의미 유사도 — "친환경 정책"≈"녹색 성장"
- **하이브리드**: 렉시컬+벡터 결합. 결합방식 2종(점수 가중 default · 순위 결합 rrf)
- 작업 4종: 검색 / RAG / Text-to-SQL / Text-to-SQL with Chart. 결과 개수 5·10·20·50(단 RAG는 컨텍스트 10건으로 제한).
- 예시(정책 도메인): 렉시컬="중대재해처벌법 시행령" / 벡터="기업 친환경 전환 정부 지원" / 하이브리드="2026년 소상공인 R&D 세액공제 변경"

### 온톨로지 실습 (/graph-lab)
SEC EDGAR 13F-HR(2025Q3, 유명 자산운용사 서브셋, 기관 약 64개)를 Neo4j에 적재 후, 같은 질문을 **RAG / Text-to-SQL / GraphRAG 3패널 동시 비교**. 사용 전 그래프 구축 필수(셋업 중 curl 로 자동 완료).
- RAG: 텍스트 유사도 → 표면적 답변
- Text-to-SQL: 단일 테이블 쿼리 → 정확한 수치(대조군)
- GraphRAG: Neo4j 멀티홉 경로 → "MS와 NVIDIA를 동시 보유한 기관" 같은 연결 질문에서 압도적 우월
- 예시: "뱅가드 보유 상위 종목 10개" / "마이크로소프트와 엔비디아를 동시 보유한 기관투자자" / "블랙록과 뱅가드의 공통 보유 종목"
- ⚠️ Neo4j 는 Docker 자동 보장. Docker Desktop 미실행이면 "Neo4j 준비 실패".

### 저장소 탐색기 (/store-explorer)
앱이 보유한 **모든 내부 저장소**를 한곳에서 들여다보는 3단계 드릴다운 화면(읽기 + 삭제).
- **두 저장소 통합**: OpenSearch 색인(searchlab-*, 문서 검색·RAG 색인) + SQLite 테이블(sqllab_*, Text-to-SQL 적재 데이터)을 종류별 섹션으로 나열(라벨·식별자·문서/행 수).
- **드릴다운**: 인덱스/테이블 클릭 → 문서/행 목록 → 상세 데이터. OpenSearch 는 색인된 도큐먼트(청크·임베딩·메타 라벨 포함)를 ◀N/M▶ 로, SQLite 는 적재 테이블 앞 N행을 표로.
- **삭제/초기화**: 각 항목에서 인덱스 삭제(searchlab-* 만)·테이블 초기화(sqllab_* 만) — 확인 모달 후 실행, 목록 자동 갱신. (도메인 색인·데이터 적재 메뉴의 삭제와 동일 동작을 통합 노출)
- ⚠️ 백엔드는 기존 색인/적재 API 를 재사용한다(신규 저장소 없음). OpenSearch 미기동이면 색인 섹션만 안내가 뜨고 SQLite 섹션은 정상 표시.

## 5개 도메인 & 직군 매핑
- 상권(sangkwon): 유통·소상공인
- 의료/제약(medical): 의료·제약
- 금융/연금/고용(finance): 금융·투자
- 법률/법령(legal): 법률·규제
- 정책/거버넌스(policy): 공공·정책
데이터 소스: GitHub raw (hanfeni/aiceo-4th-training/main/poc/data/<domain>). 검색=jsonl, SQL=csv, graph-lab=sec-edgar.

## 권장 실습 순서
챗 에이전트 → 도메인 색인 → 검색 실습 → (데이터 적재 →) 온톨로지 실습 → 하네스 구성

## 자주 묻는 질문
Q. 색인이 안 되어 있으면 검색이 안 되나요? → 네. /index-lab 에서 먼저 색인하세요(챗 인덱스검색도 동일).
Q. graph-lab 이 느린데 정상인가요? → 첫 구축 시 1~3분 정상입니다.
Q. DART 가 에러나요. → DART_API_KEY 미입력이면 분석 실행 시 에러. opendart.fss.or.kr 무료 발급 후 .env.local 에 입력.
Q. 챗에서 도메인 선택하면 뭐가 달라지나요? → 도구 미선택=고정형 챗 ↔ 선택=자율형(에이전트가 검색·SQL 도구 스스로 호출) 차이를 체감.
Q. 더 정확한 답을 원하면? → 에이전트가 코드를 직접 분석해 답해드립니다. "이 옵션 값이 정확히 뭐예요?", "왜 이렇게 동작하죠?" 처럼 물어보세요.
---
...
| Docker 설치는 됐는데 `docker info` 가 계속 실패 | **첫 설치 후 첫 기동 동의 관문 미통과** → Docker Desktop 화면에서 ① 약관 `Accept` ② 기본설정 `Finish/Continue` ③ macOS 권한 `OK` ④ 로그인 `Skip` 을 사람이 끝내야 데몬이 뜬다. 메뉴바 고래 아이콘이 `running` 이 되면 재시도 |
| graph-lab 구축 `HTTP 4xx/5xx` | Docker Desktop 미실행 → 실행 후 메뉴바 아이콘 'running' 확인하고 curl 재실행 (처음 설치라면 위의 동의 관문부터 통과) |
