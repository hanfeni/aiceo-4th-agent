...
### 1.11 사고 패널 — 구현 반영 (Addendum, 원 §1.6/§1.8 보강)

> 본 절은 thinking-panel 작업(feat/deepagents-chat-harness) 이후 추가된
> **구현 사실 기록**이다. 원 §1.6(SSE 3-이벤트 계약)·§1.8(에이전트 라벨)
> 및 AD-1~AD-6 결정 트레일은 그대로 보존하고, 아래는 그 위에 누적된
> 실제 동작을 명시해 향후 독자/에이전트의 오인을 막는다. FR-09/R5(본문
> 누출 0)는 사고를 **별도 채널**로 분리함으로써 그대로 유지된다(약화 아님).

- **AD-7 (SSE 이벤트 확장 — 원 §1.6 보강):** `SseEvent` union 은 3종
  (`thread`/`token`/`done|error`)이 아니라 **7종**으로 확정:
  `thread | token | thinking | tool_call | tool_result | done | error`
  (`src/types/index.ts`). `thinking` 은 reasoning 텍스트, `tool_call`
  은 도구 호출 IN(args 점진 누적), `tool_result` 는 도구 실행 OUT.
  route emit 순서: `thread` → (`token`|`thinking`|`tool_call`|
  `tool_result`)* → `done`|`error`. `token` 스트림에는 thinking·tool
  이 0건(FR-09 불변) — chunkFilter 가 본문만, extractThinking/
  extractToolCalls/extractToolOutputs 가 각 채널을 분리 수집한다.

- **AD-8 (사고 패널 — 원 §1.8 "에이전트 라벨" 대체):** assistant 메시지
  헤더의 "에이전트" 라벨은 제거되고 `ThinkingPanel`
  (`src/components/common/BaseChat/ThinkingPanel.tsx`)로 교체됐다.
  `MessageList` 가 `thinkingSteps[]` 를 패널에 주입한다(라벨 위치 동일).
  스트리밍 중엔 마지막 step 만 리플레이스 표시(자동 펼침), 완료/사용자
  토글 시 전체 step 누적 표시(medigate-new StreamingView/HistoryView
  패턴). 사고 텍스트도 ChatMarkdown rehype-sanitize 경유(XSS/FR-09).

- **AD-9 (thinkingSteps[] 모델 — 원 §1.7 ChatMessage 보강):**
  `ChatMessage.thinkingSteps?: ThinkingStep[]` 추가. reasoning step 과
  tool step 이 **단일 배열에 발생 순서대로** 섞여 교차(사고→도구→사고
  →도구)가 보존된다. reasoning step 은 `**bold 제목**` 으로 분기,
  tool step 은 `tool_call`(IN) push → `tool_result`(OUT) 로 채움(id
  매칭). `useChat` 의 SSE 매핑이 store reducer 로 누적한다.

- **AD-10 (OpenAI reasoning 활성화 조건 — model.ts):** GPT-5 계열에서
  reasoning 텍스트(사고 패널 데이터원)를 받으려면 `ChatOpenAI` 에
  `useResponsesApi:true` + `reasoning:{effort:"medium",summary:"auto"}`
  필수다. Chat Completions API 는 reasoning 을 카운트만 하고 텍스트를
  반환하지 않는다(Slice 1 실측엔 reasoning 미발생으로 관측됐으나, 본
  설정 추가 후 Responses API summary 블록으로 토큰 스트리밍됨). 이
  provider별 차이는 model.ts 가 흡수한다(FR-10 불변).

- **AD-11 (web_search ServerTool 채널 — chunkFilter):** OpenAI built-in
  `web_search` 는 ClientTool 의 `tool_call_chunks` 가 **아니라**
  model_request 청크의 `additional_kwargs.tool_outputs[]`
  (`{type:"web_search_call", action:{queries}}`)로 온다. 별도
  `extractToolOutputs` 가 이 채널을 수집해 `tool_call`+`tool_result`
  로 정규화한다(ClientTool 과 동일 UI 채널로 통일, agent.ts/UI 무수정).
