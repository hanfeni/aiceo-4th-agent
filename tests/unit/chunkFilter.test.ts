    ...
    expect(serverOutputs).toEqual([
      {
        ...
        name: "web_search",
        args: JSON.stringify({
          actions: [{ type: "search", queries: ["q1", "q2"] }],
        }),
        result: undefined,
      },
    ]);
...
describe("extractToolOutputs — ServerTool 호출 추출 (FR-09 거울상, 채널 단독 전담)", () => {
  ...
  it("web_search status='completed' 여도 result=undefined(OUT=결과)", () => {
    ...
    // S1 새 포맷: action 을 {actions:[{type, ...raw필드}]} 로 감싼다
    // (그룹 누적은 reducer 책임 — extractToolOutputs 는 청크당 1 action).
    expect(extractToolOutputs(msg, meta())).toEqual([
      {
        id: "ws_1",
        name: "web_search",
        args: JSON.stringify({
          actions: [{ type: "search", queries: ["a 검색", "b query"] }],
        }),
        result: undefined,
      },
    ]);
  ...
  // RAW 실측(WS_RAW_PROBE): web_search 는 action.type 이 "search"
  // (queries 보유) 외에 "open_page"(url 보유, queries 없음)도 온다.
  // 기존 코드는 queries 만 봐서 open_page 가 args="" 빈 step 으로 떨어짐
  // (사고 패널에 '어느 페이지를 열어 읽었나' 누락). open_page 는 url 을
  // args 에 담아 표시한다 — queries 와 대칭 구조(JSON, 별도 키).
  it("open_page 액션: action.url 을 args 에 담는다(검색 결과 페이지 열람 표시)", () => {
    const msg = {
      kwargs: {
        additional_kwargs: {
          tool_outputs: [
    ...
    expect(extractToolOutputs(msg, meta())).toEqual([
      {
        ...
        name: "web_search",
        args: JSON.stringify({
          actions: [{ type: "open_page", url: "https://openai.com/news/" }],
        }),
        result: undefined,
      },
    ]);
  ...
  // R8 graceful passthrough: action 의 type 외 모든 필드를 그대로 보존.
  // find_in_page(실측: {type,pattern,url})·미래 새 action.type 무수정 대응.
  it("find_in_page: type 외 필드(pattern,url) 통째 보존 (실측 RAW)", () => {
    const msg = {
      kwargs: {
        additional_kwargs: {
          tool_outputs: [
            {
              ...
              action: {
                type: "find_in_page",
                pattern: "Consolidated Revenue",
                url: "https://news.samsung.com/x",
              },
            },
          ],
        },
  ...
  it("미지 action.type 도 graceful passthrough (하드코딩 0 — R8)", () => {
    const msg = {
      kwargs: {
        additional_kwargs: {
          tool_outputs: [
            {
              id: "ws_future",
              type: "web_search_call",
              action: { type: "some_new_action", foo: "bar", n: 7 },
            },
          ],
        },
      },
    ...
    expect(extractToolOutputs(msg, meta())).toEqual([
      {
        ...
        name: "web_search",
        args: JSON.stringify({
          actions: [{ type: "some_new_action", foo: "bar", n: 7 }],
        }),
        result: undefined,
      },
    ]);
  ...
  it("search 와 open_page 가 섞여 와도 각각 올바른 args (queries vs open_page)", () => {
    const msg = {
      kwargs: {
        additional_kwargs: {
          tool_outputs: [
    ...
    expect(extractToolOutputs(msg, meta())).toEqual([
      {
        ...
        name: "web_search",
        args: JSON.stringify({
          actions: [{ type: "search", queries: ["삼성전자 005930"] }],
        }),
        result: undefined,
      },
      {
        ...
        name: "web_search",
        args: JSON.stringify({
          actions: [{ type: "open_page", url: "https://x.com/p" }],
        }),
        result: undefined,
      },
    ]);
  ...
  // graceful passthrough 의도: action 이 **존재하면** type 외 필드가
  // 비정상(url:123)이어도 action 자체는 보존한다(정보 손실 0 — R8).
  // 이전엔 url 비-string 이면 args="" 로 버렸으나 그건 "open_page 가
  // 있었다"는 사실 손실. 표시 측(UI)이 비정상 url 을 graceful 처리.
  it("open_page url 이 비-string 이어도 action 통째 보존 (손실 0)", () => {
    const msg = {
      kwargs: {
        additional_kwargs: {
          tool_outputs: [
    ...
    expect(extractToolOutputs(msg, meta())).toEqual([
      {
        id: "ws_bad",
        name: "web_search",
        args: JSON.stringify({ actions: [{ type: "open_page", url: 123 }] }),
        result: undefined,
      },
    ]);
  });
