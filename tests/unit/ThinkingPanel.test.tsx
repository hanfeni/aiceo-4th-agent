...
describe("ThinkingPanel — '(진행 중)' 깜빡임 제거(폴백 라벨)", () => {
  const steps = [reasoning("질문 분석 중", "본문", 0)];

  it("스트리밍 중 첫 tick 전(cyclingLabel 빈값) 폴백은 '답변 과정' — '(진행 중)' 없음", () => {
    // useThinkingLabelCycler 는 첫 tick(80ms) 전 빈 문자열 →
    // 폴백 '답변 과정'(정적 '(진행 중)' 제거). render 직후 검증.
    const { container } = render(
      <ThinkingPanel steps={steps} streaming={true} />,
    );
    expect(container.textContent).toContain("답변 과정");
    expect(container.textContent).not.toContain("진행 중)");
    expect(container.textContent).not.toContain("(진행");
  });

  it("완료(streaming=false) 라벨도 '답변 과정'(괄호 표현 없음)", () => {
    const { container } = render(
      <ThinkingPanel steps={steps} streaming={false} />,
    );
    expect(container.textContent).toContain("답변 과정");
