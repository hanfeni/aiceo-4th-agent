...
// ── Slice J: Task(Agent) 위임 — deepagents `task` 도구 ───────────────
// 실측(node_modules/deepagents index.js): subagent 호출은 name="task",
// args = {description, subagent_type}. medigate-new 동형으로
// '{한글라벨} 에이전트 실행 중' → 완료 '… 에이전트 완료'.
// subagent_type→한글은 subagent 파일 *DisplayName 수집, 미매핑 폴백.
describe("toolTitle — name='task' 면 Agent 위임 표현", () => {
  it("task + subagent_type=web-searcher, 진행 중 → '웹 검색 에이전트 실행 중'", () => {
    const args = JSON.stringify({
      description: "삼성전자 검색",
      subagent_type: "web-searcher",
    });
    expect(toolTitle("task", false, args)).toBe(
      "웹 검색 에이전트 실행 중",
    );
  });

  it("task + web-searcher, 완료 → '웹 검색 에이전트 완료'", () => {
    const args = JSON.stringify({ subagent_type: "web-searcher" });
    expect(toolTitle("task", true, args)).toBe("웹 검색 에이전트 완료");
  });

  it("미매핑 subagent_type → 원본명 폴백 '{type} 에이전트 실행 중'", () => {
    const args = JSON.stringify({ subagent_type: "custom-agent" });
    expect(toolTitle("task", false, args)).toBe(
      "custom-agent 에이전트 실행 중",
    );
  });

  it("args 가 불완전 JSON(스트리밍 중) → 이름 없이 '에이전트 실행 중'", () => {
    expect(toolTitle("task", false, '{"description":"부분')).toBe(
      "에이전트 실행 중",
    );
  });

  it("args 미전달(undefined) → '에이전트 실행 중' 폴백", () => {
    expect(toolTitle("task", false)).toBe("에이전트 실행 중");
  });

  it("subagent_type 없는 args → '에이전트 실행 중'(이름 없음)", () => {
    expect(toolTitle("task", true, '{"description":"x"}')).toBe(
      "에이전트 완료",
    );
  });

  it("name='task' 가 아니면 기존 도구 표현 유지(args 무시)", () => {
    const args = JSON.stringify({ subagent_type: "web-searcher" });
    expect(toolTitle("web_search", false, args)).toBe(
      "웹 검색 도구 실행 중",
    );
  });
});
